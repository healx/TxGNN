...
def construct_negative_graph(graph, k, device, method="fix_dst", weights=None):
    if weights is None:
        if method == "multinomial_src":
            weights = {
                etype: graph.out_degrees(etype=etype).float() ** 0.75
                for etype in graph.canonical_etypes
            }
        elif method == "multinomial_dst":
            weights = {
                etype: graph.in_degrees(etype=etype).float() ** 0.75
                for etype in graph.canonical_etypes
            }
            ...
            weights = {
                etype: (graph.in_degrees(etype=etype) > 0).float()
                for etype in graph.canonical_etypes
            }
        elif method == "fix_src":
            weights = {
                etype: (graph.out_degrees(etype=etype) > 0).float()
                for etype in graph.canonical_etypes
            }
        else:
            weights = {}
    ...
    for etype in graph.canonical_etypes:
        out.update(
            construct_negative_graph_each_etype(
                graph, k, etype, method, weights, device
            )
        )
...
class Minibatch_NegSampler(object):
    def __init__(self, g, k, method, device=None):
        ...
        if self.device is not None:
            self.weights = {
                etype: w.to(self.device) for etype, w in self.weights.items()
            }
        self.k = k
