...
class TxGNN:
    ...
    def pretrain(
        ...
        edge_sampler = dgl.dataloading.as_edge_prediction_sampler(
            sampler,
            negative_sampler=Minibatch_NegSampler(
                self.G, 1, "fix_dst", device=dataloader_device
            ),
        )
    ...
    def retrieve_save_gates(self, path, relation=None, no_base=False):
        if relation is None:
            if "relation" in self.__dict__:
                ...
                raise ValueError(
                    "relation is required (or call train_graphmask first)."
                )
        ...
        all_att_df.to_pickle(
            os.path.join(path, "graphmask_output_" + relation + ".pkl")
        )
        return all_att_df
