...
class TxData:
    def __init__(self, data_folder_path):
        ...
        data_download_wrapper(
            "https://dataverse.harvard.edu/api/access/datafile/7144484",
            os.path.join(self.data_folder, "kg.csv"),
        )
        data_download_wrapper(
            "https://dataverse.harvard.edu/api/access/datafile/7144482",
            os.path.join(self.data_folder, "node.csv"),
        )
        data_download_wrapper(
            "https://dataverse.harvard.edu/api/access/datafile/7144483",
    ...
    def prepare_split(
        ...
        if split == "disease_eval":
            split_data_path = os.path.join(
                self.data_folder, self.split + "_" + str(disease_eval_idx)
            )
        elif split == "downstream_pred":
            split_data_path = os.path.join(
                self.data_folder, self.split + "_downstream_pred"
            )
            disease_eval_idx = [
                11394.0,
                6353.0,
            ...
            split_data_path = os.path.join(
                self.data_folder, self.split + "_no_kg_" + str(seed)
            )
        elif test_size != 0.05:
            split_data_path = (
                os.path.join(self.data_folder, self.split + "_" + str(seed))
                + "_frac"
                + str(test_size)
            )
        elif one_hop:
            ...
            split_data_path = os.path.join(
                self.data_folder, self.split + "_" + str(seed)
            )
        ...
        if not os.path.exists(os.path.join(split_data_path, "train.csv")):
            ...
            df_train, df_valid, df_test = create_split(
                df, split, disease_eval_idx, split_data_path, seed
            )
        else:
    ...
    def retrieve_id_mapping(self):
        ...
        id2name_disease.update(
            dict(
                df_[df_.y_type == "disease"][["y_id", "y_name"]]
                .drop_duplicates()
                .values
            )
        )
