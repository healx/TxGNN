...
class DataSplitter:
    def __init__(self, kg_path=""):
        ...
        self.edge_index = torch.LongTensor(
            self.edges.get(["x_index", "y_index"]).values.T
        )
        ...
        self.grouped_diseases = pd.read_csv(
            os.path.join(dirname, "kg_grouped_diseases_bert_map.csv")
        )
    ...
    def get_nodes_for_doid(self, code):
        ...
        doids.add(code)
        mondo = (
            self.mondo_xref.query('ontology == "DOID" and ontology_id in @doids')
            .get(["mondo_id"])
            .drop_duplicates()
            .values.reshape(-1)
            .astype("str")
    ...
    def get_one_hop_edge_group(self, nodes, mask_ratio=0.1, add_drug_dis=True):
        ...
        subgraph_nodes, filtered_edge_index, node_map, edge_mask = k_hop_subgraph(
            list(nodes), 1, self.edge_index
        )  # one hop
        ...
        sample_idx = np.random.choice(
            filtered_edge_index.shape[1], num_of_mask_edges, replace=False
        )
    ...
    def get_edge_group(self, nodes, test_size=0.05, add_drug_dis=True):
        ...
        subgraph_nodes, filtered_edge_index, node_map, edge_mask = k_hop_subgraph(
            list(nodes), 2, self.edge_index
        )  # one hop neighborhood
        print("filtered_edge_index.shape: ", filtered_edge_index.shape)
        sample_idx = np.random.choice(
            filtered_edge_index.shape[1], num_random_edges, replace=False
        )
    ...
    def get_test_kg_for_disease(
        ...
        if one_hop:
            disease_edges = self.get_one_hop_edge_group(
                disease_nodes, mask_ratio=mask_ratio, add_drug_dis=add_drug_dis
            )
        else:
            disease_edges = self.get_edge_group(
                disease_nodes, test_size=test_size, add_drug_dis=add_drug_dis
            )
